...
Multiset::Multiset(const int x) // Create an empty multiset. 
{
	if (x <= 0)
	{
		std::cerr << "Cannot create a Multiset with "
			<< x << std::endl;
		exit(1);
	}
	m_count = 0;
	m_array = new distinct[x];
	m_x = x;
}
...
{
	if (!contains(value) && uniqueSize() + 1 > m_x)
		return false;
